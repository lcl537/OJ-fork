...
async def upload_file(request: Request, file: UploadFile = File(...)):
    ...
    file_path = base_dir / f"{time.time_ns()}.py"
    try:
    ...
    try:
        ...
        cursor.execute(
            ...
            (file_path.name, client_ip, db_config['password'], timestamp, timestamp, "SUBMITTED")
        )
        conn.commit()
...
@app.patch("/update_status/{id}")
async def update_status(id: int, payload: dict):
    ...
    try:
        ...
        cursor.execute(
            "UPDATE files SET status = %s, updated_at = %s WHERE id = %s",
            (status, timestamp, id)
        )
        conn.commit()
        cursor.close()
